...
const {
  ...
  dbUpdatePlayer,
  dbGetAllPlayers,
  dbGetPlayerBySocketID,
  dbUpdateRoom,
  dbNewResult,
  dbGetresultByRoomID,
  dbDeleteResultByRoomId,
} = require("./dbActions");
        ...
        if (room.status == "open") {
          dbGetPlayerByID(db, data.playerID).then((result) => {
            ...
            if (player) {
              ...
              dbUpdatePlayer(db, updatePlayer)
                .then((result) => {
                  callback({
                    status: true,
                    data: {
...
const removePlayerFromRoom = async (socketID, callback) => {
  ...
  dbGetPlayerBySocketID(db, socketID)
    .then((result) => {
      // console.log(result)
      const player = result;
      if (player) {
        dbGetRoomByID(db, player.room.roomID)
          .then((result) => {
            const room = result;
            const newPlayer = {
              ...player,
...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", (data) => {
    startGame(
      data,
      ({ status: startStatus, error: startError, data: startData }) => {
    ...
    dbGetRoomByID(db, data.room.roomID).then((result) => {
      ...
      if (result) {
        ...
        data.eliminated.forEach((player) => {
          const findPlayer = data.room.players.find(function (el) {
        ...
        dbDeleteResultByRoomId(db, newResult.roomID)
          .then((result) => {
